...
def complete_plain_list(diff_dict, diff_list: list, path=[], depth_lvl=1):
    ...
    for key in sorted_keys:
        ...
        if depth_lvl == 1:
            path = []
        if key_type == 'changed':
            old_value = convert_to_plain(diff_dict[key]['old_value'])
            new_value = convert_to_plain(diff_dict[key]['new_value'])
            diff_list.append(f'Property \'{"".join(path)}{key}\' '
                             f'was updated. From {old_value} to {new_value}')
        else:
            value = convert_to_plain(diff_dict[key]['value'])
            if key_type == 'nested':
                path.append(f'{key}.')
                children = diff_dict[key]['value']
                complete_plain_list(children, diff_list, path, depth_lvl + 1)
            if key_type == 'deleted':
