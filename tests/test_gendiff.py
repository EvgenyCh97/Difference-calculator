...
def test_get_dicts_from_():
    result = gendiff.get_dicts_from_(JSON1, JSON2)
...
def test_get_keys_set_from_(json_dict1, json_dict2):
    ...
    assert result == ({'proxy', 'timeout', 'follow', 'host'},
                      {'timeout', 'verbose', 'host'})
...
def test_get_updated_lines():
    ...
    assert set(result[0]) == {('- timeout', 50)}
    assert set(result[1]) == {('+ timeout', 20)}
